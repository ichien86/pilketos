  ...
  return (
    <main className="min-h-screen p-4 max-w-md mx-auto space-y-4">
      ...
      <form onSubmit={simpan} className="bg-white rounded-xl shadow p-4 space-y-3">
        <div>
          <label className="text-sm font-medium block mb-1">Alamat rumah</label>
          <textarea className="w-full border rounded-lg px-3 py-2" value={alamat} onChange={(e) => setAlamat(e.target.value)} rows={2} />
        </div>
        <div>
          <label className="text-sm font-medium block mb-1">Hobi</label>
          <input className="w-full border rounded-lg px-3 py-2" value={hobi} onChange={(e) => setHobi(e.target.value)} />
        </div>
