...
- **Format Excel DPT** (US-01): sheet `Siswa` dengan kolom header persis `NIS`, `Nama`, `Kelas`, `Tanggal Lahir`; sheet `Guru` dengan `NIP`, `Nama`, `Pangkat`, `Tanggal Lahir`. Kolom tanggal lahir boleh berupa sel tanggal Excel asli, atau teks dalam salah satu format: `YYYY-MM-DD`, `DD/MM/YYYY`, `DD-MM-YYYY`, `DD.MM.YYYY`, atau `DD <Nama Bulan> YYYY` (nama bulan Indonesia atau Inggris, penuh maupun singkatan -- mis. "17 Agustus 2008" atau "17 Aug 2008"). Tanggal yang tidak masuk akal (mis. 31 Februari) maupun format lain di luar daftar ini ditolak sebagai error per baris, bukan ditebak. **Format angka (garis miring/strip/titik) yang hari dan bulannya sama-sama <=12 (mis. "9/12/1986") SENGAJA ditolak** -- ambigu antara DD/MM dan MM/DD ala Amerika, dan salah tebak di sini akan bikin pemilih yang bersangkutan gagal aktivasi terus (tanggal lahir tidak cocok, US-02). Panitia harus menulis ulang pakai nama bulan atau format ISO untuk kasus itu. Baris dengan kolom kosong atau NIS/NIP duplikat ditolak saat commit.
- **Password default** (`DEFAULT_PASSWORD`, default `MAN3Byl`): sama untuk semua akun pemilih sebelum aktivasi — **wajib** diganti sebelum masa pendataan dimulai kalau nilai default ini pernah bocor ke publik sebelumnya. Reset password oleh panitia/admin (US-04) juga selalu mengembalikan ke nilai default yang sama ini (bukan password acak) -- pemilih mengulang alur aktivasi (US-02) dari awal, termasuk cek tanggal lahir.
