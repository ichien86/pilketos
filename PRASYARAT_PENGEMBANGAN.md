...
6. **Pendataan tidak bisa ditutup** selagi masih ada pemilih di DPT yang belum mengaktivasi akunnya, dan **sosialisasi tidak bisa ditutup** selagi masih ada pemilih yang belum menonton semua video kandidat wajib — **tidak ada jalur pengecualian**, sesuai keputusan eksplisit saat fitur ini diminta. Panitia bisa memantau siapa saja yang masih kurang lewat filter "belum aktivasi" / "belum sosialisasi" di `/admin/dpt` sebelum mencoba menutup fase-nya. Karena tidak ada pengecualian, pastikan seluruh DPT benar-benar bisa 100% aktivasi/nonton sebelum hari-H -- kalau ada yang secara realistis tidak akan pernah bisa (keluar sekolah, sakit permanen, dll), hapus datanya dari DPT lebih dulu (lihat CRUD di `/admin/dpt`), bukan berharap gerbang ini dilewati.
